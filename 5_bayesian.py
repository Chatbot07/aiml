...
def joint_probability(alarm, burglary, earthquake, david_calls, sophia_calls):#(t,f,f,t,t)
    ...
    if alarm else 
    (P_david_calls_given_no_alarm if david_calls else P_david_does_not_call_given_no_alarm)#0.91
    
    P_sophia = (P_sophia_calls_given_alarm if sophia_calls else P_sophia_does_not_call_given_alarm)
    if alarm else 
    (P_sophia_calls_given_no_alarm if sophia_calls else P_sophia_does_not_call_given_no_alarm)#0.75
